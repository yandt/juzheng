...
把本机变成局域网代理,供手机、平板等同网段设备使用:

1. 打开**设置 → 网络与代理 → 局域网共享**,开启「共享代理」。
2. 设置监听端口(默认 `7890`);在公共网络建议同时填用户名/密码。
3. 保存后**重启内核**生效。
4. 其他设备把 HTTP/SOCKS5 代理设为「**本机局域网 IP : 端口**」(如 `192.168.1.10:7890`)即可经本机出网。

> 共享代理会向局域网开放端口,请仅在可信网络下使用。
